...
{    
    ...
    function index($dept=null)
    {
        ...
        $offset = ($page-1)*$rows;      
        $sort   = isset($_POST['sort']) ? strval($_POST['sort']) : 'fcuti_id';
	...
	if (!empty($filterRules)){
        ...
        $this->db->join(self::$table3.' b', 'a.fcuti_bagian = b.departemen_id', 'left')
                 ->join(self::$table3.' c', 'b.departemen_induk = c.departemen_id', 'left')
                 ->join(self::$table2.' d', 'a.fcuti_nik = d.karyawan_nik', 'left')
                 ->join(self::$table4.' e', 'a.fcuti_disetujui = e.id', 'left')
                 ->join(self::$table4.' f', 'a.fcuti_diketahui = f.id', 'left')
                 ->join(self::$table4.' g', 'a.fcuti_ditolak = g.id', 'left')
                 ->join(self::$table4.' h', 'a.fcuti_pembuat = h.id', 'left');
    ...
    function create($fcuti_tanggal, $fcuti_nik, $fcuti_bagian, 
                    $fcuti_dari, $fcuti_sampai, $fcuti_keperluan,
                    $fcuti_keterangan, $user_id)
    {    
        return $this->db->insert(self::$table1,array(
            'fcuti_tanggal'     => $fcuti_tanggal,
            'fcuti_nik'         => $fcuti_nik,
            'fcuti_bagian'      => $fcuti_bagian,
            'fcuti_dari'        => $fcuti_dari,
            'fcuti_sampai'      => $fcuti_sampai,
            'fcuti_keperluan'   => $fcuti_keperluan,
            'fcuti_keterangan'  => $fcuti_keterangan,
            'fcuti_pembuat'     => $user_id
        ));
    }
    
    function update($fcuti_id, $fcuti_tanggal, $fcuti_nik, $fcuti_bagian, 
                    $fcuti_dari, $fcuti_sampai, $fcuti_keperluan, $fcuti_keterangan)
    {
        $this->db->where('fcuti_id', $fcuti_id);
    ...
    function delete($fcuti_id)
    {
        return $this->db->delete(self::$table1, array('fcuti_id' => $fcuti_id)); 
    }
    ...
    function disetujui($fcuti_id, $fcuti_disetujui)
    {
        $this->db->where('fcuti_id', $fcuti_id);
        return $this->db->update(self::$table1,array(
            'fcuti_disetujui' => $fcuti_disetujui
        ));
    }
    
    function diketahui($fcuti_id, $fcuti_diketahui)
    {
        $this->db->where('fcuti_id', $fcuti_id);
        return $this->db->update(self::$table1,array(
            'fcuti_diketahui' => $fcuti_diketahui
        ));
    }
    
    function ditolak($fcuti_id, $fcuti_ditolak, $fcuti_keterangan)
    {
        $this->db->where('fcuti_id', $fcuti_id);
        return $this->db->update(self::$table1,array(
            'fcuti_ditolak'     => $fcuti_ditolak,
            'fcuti_keterangan'  => $fcuti_keterangan
        ));
    }
}
