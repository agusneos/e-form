...
{    
	...
	if (!empty($filterRules)){
        ...
        $this->db->join(self::$table3.' b', 'a.fabsenman_bagian = b.departemen_id', 'left')
                 ...
                 ->join(self::$table2.' d', 'a.fabsenman_nik = d.karyawan_nik', 'left')
                 ->join(self::$table4.' e', 'a.fabsenman_disetujui = e.id', 'left')
                 ->join(self::$table4.' f', 'a.fabsenman_diketahui = f.id', 'left')
                 ->join(self::$table4.' g', 'a.fabsenman_ditolak = g.id', 'left')
                 ->join(self::$table4.' h', 'a.fabsenman_pembuat = h.id', 'left');
    ...
    {    
        return $this->db->insert(self::$table1,array(
            ...
            'fabsenman_keterangan'  => $fabsenman_keterangan,
            'fabsenman_pembuat'     => $user_id
        ));
    }
    
    function update($fabsenman_id, $fabsenman_tanggal, $fabsenman_nik,
    ...
    function disetujui($fabsenman_id, $fabsenman_disetujui)
    {
        $this->db->where('fabsenman_id', $fabsenman_id);
        return $this->db->update(self::$table1,array(
            'fabsenman_disetujui' => $fabsenman_disetujui
        ));
    }
    
    function diketahui($fabsenman_id, $fabsenman_diketahui)
    {
        $this->db->where('fabsenman_id', $fabsenman_id);
        return $this->db->update(self::$table1,array(
            'fabsenman_diketahui' => $fabsenman_diketahui
        ));
    }
    
    function ditolak($fabsenman_id, $fabsenman_ditolak, $fabsenman_keterangan)
    {
        $this->db->where('fabsenman_id', $fabsenman_id);
        return $this->db->update(self::$table1,array(
            'fabsenman_ditolak'     => $fabsenman_ditolak,
            'fabsenman_keterangan'  => $fabsenman_keterangan
        ));
    }
}
